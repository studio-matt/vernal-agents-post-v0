...
class AssetLoader:
    ...
    def load_liwc_baselines(self) -> dict[str, dict[str, float]]:
        means = {
            row["Category"]: float(row["Mean"]) for row in self.read_csv("LIWC_Mean_Table.csv", ["Category", "Mean"])
        }
        stdevs = {
            row["Category"]: float(row["StdDev"])
            for row in self.read_csv("LIWC_StdDev_Mean_Table.csv", ["Category", "StdDev"])
        }
