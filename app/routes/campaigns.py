...
def get_campaigns(current_user = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Demo campaign is created only at signup (auth_api). We do NOT re-create it on GET:
        # if a user deletes their demo campaign, it stays deleted and does not affect other users.

        # Sort campaigns: demo campaign first (if present), then by created_at
