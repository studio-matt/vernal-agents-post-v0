...
def get_campaigns(current_user = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Demo campaign is created only at signup (auth_api). We do NOT re-create it here:
        # if the user deletes their demo campaign, it stays deleted.

        # Sort campaigns: demo campaign first (if any), then by created_at
