...
Use these names consistently: **Front End Server** / **Back End Server** / **WordPress** (plugin).

**All three repos exist in this workspace:**
- **Frontend:** workspace root → `vernal-post-v0`. Path: `.` or `/home/runner/workspace`
- **Backend:** `backend-repo-git/` → `vernal-agents-post-v0`. Path: `backend-repo-git/` or `/home/runner/workspace/backend-repo-git`
- **WordPress (plugin):** `vernal-wordpress-plugin/` → WordPress plugin repo. Path: `vernal-wordpress-plugin/`

---

## 🚨 BACKEND IS IN THIS WORKSPACE
**The backend repo lives in this workspace at `backend-repo-git/`.**  
- **Path:** `/home/runner/workspace/backend-repo-git` (or `./backend-repo-git` from workspace root)  
- **Commit and push backend changes from there** — same workspace, separate git repo from frontend.  
- Do not assume the backend is "on the server only" or "not in the workspace."
...
- **Repo checkout (server):** `/home/ubuntu/vernal-agents-post-v0`
- **In this workspace:** `backend-repo-git/` — make edits here, commit, push to same GitHub repo
...
### **Content creation vs posting (all platforms) — principle**
- **Content creation** (generate-content, writing/QC agents): Only author personality, brand voice, platform **guidelines**, custom/writing/QC agents. No platform **posting** APIs (LinkedIn, WordPress, etc.) must be involved or block this step.
- **Posting** (post-now, Publish Day/Week/Entire): Platform APIs (LinkedIn, WordPress, etc.) and credentials apply only here. LinkedIn author URN, WordPress connection, etc. are irrelevant to the content-creation step and must not hold it up.
...
- **Frontend:** Uses 30s timeout for GET campaigns; has a 35s failsafe to stop the spinner. On error it shows cached data if available or a toast. Ensure backend at `https://themachine.vernalcontentum.com` is up and responding.
- **Checks:** `curl -s -o /dev/null -w "%{http_code}" https://themachine.vernalcontentum.com/health` (expect 200). On the server: `curl -s http://127.0.0.1:8000/health`.

### **LinkedIn post-now 422 (Publish Day / Week / Entire campaign)**
- **Error:** `POST .../post-now` returns 400 with LinkedIn API 422: `"urn:li:person:1" does not match urn:li:company:\d+|urn:li:member:\d+`
- **Root Cause:** Backend (or LinkedIn integration) is sending a **person** URN as author; LinkedIn’s **post** API requires a **company** or **member** URN. This is a **posting**-phase concern only; it must not affect generate-content.
- **Symptoms:** Publish Day’s Content, Publish Week’s Content, Publish Entire Campaign all fail with the same message.
- **Fix (backend):** Use `urn:li:company:<id>` or `urn:li:member:<id>` for the author when calling LinkedIn’s API, not `urn:li:person:<id>`. Ensure the connected LinkedIn account / OAuth token is for a Company or Member page, not a personal profile.
- **Frontend:** Shows a short, actionable message and does not fix the URN; the backend or LinkedIn app config must be corrected.
...
## 🔍 DEBUGGING BACKEND DROPPING (Timeouts, 502, Connection Resets)

**When the frontend sees:** `Network Error`, `ERR_NETWORK_CHANGED`, `timeout of 60000ms exceeded`, 502 Bad Gateway, or requests that never complete.

### 1. Confirm the service is up and listening

```bash
# On Back End Server (18.235.104.132)
sudo systemctl status vernal-agents
# Expect: active (running)

# Port 8000 must be listening
ss -tlnp | grep 8000
# or
...
If service is "active" but port 8000 is not listening, see **CRITICAL: ENVIRONMENT FILE PRESERVATION** and **DEPLOYMENT VORTEX PREVENTION** (missing `.env` or blocking at import).

### 2. Watch logs while reproducing the drop

```bash
# Leave this running, then trigger the failing action from the frontend
sudo journalctl -u vernal-agents -f

# Or filter for errors and slow endpoints
sudo journalctl -u vernal-agents -f | grep -E "ERROR|CRITICAL|timeout|generate-content|generate_image|post-now|502|500"
```

- **No log line when request is made** → Request not reaching the app (nginx/proxy, DNS, or frontend URL).
- **Log line appears then nothing** → Request reached app but handler is blocking or very slow (agent/timeout/DB).
- **Traceback / ERROR in logs** → Backend exception; fix the cause (DB, import, validation, etc.).

### 3. Nginx timeouts (502 / upstream timed out)

Long-running endpoints (e.g. `generate-content`, `generate_image_machine_content`) can exceed nginx defaults.

```bash
# On Back End Server, check nginx config
sudo nginx -T 2>/dev/null | grep -E "proxy_read_timeout|proxy_connect_timeout|proxy_send_timeout"

# Typical fix: increase timeouts in server block for themachine.vernalcontentum.com
...
### 4. Backend request timeouts

- Frontend uses 60s (or similar) for content/image generation. If the backend takes longer, the client will timeout even if the backend eventually succeeds.
- **Options:** Increase timeout in frontend for that endpoint, and/or make the backend respond sooner (e.g. return a task ID and poll for status instead of one long request).
- Check backend logs for the same request: if the backend logs success after the frontend already timed out, the fix is longer client timeout or async + polling.

### 5. Database / connection pool

- DB connection timeouts or pool exhaustion can make requests hang or fail.
- Test DB from the server:
  ```bash
  cd /home/ubuntu/vernal-agents-post-v0 && source .env 2>/dev/null; python3 -c "
  from database import DatabaseManager
  db = DatabaseManager()
  print('✅ DB connection OK')
  "
  ```
- In logs, look for `OperationalError`, `Can't connect to MySQL`, `Too many connections`, or pool-related errors.

### 6. Memory / OOM

- If the process is killed under load, journalctl may show OOM or the service may restart repeatedly.
  ```bash
  sudo journalctl -u vernal-agents --since "1 hour ago" | grep -iE "killed|oom|memory"
  free -h
  ```
- Reduce concurrency, increase server RAM, or optimize heavy endpoints (e.g. streaming, chunking, or offloading to a worker).

### 7. Quick checklist

| Check | Command |
|-------|--------|
| Service running | `sudo systemctl status vernal-agents` |
| Port 8000 | `curl -s http://127.0.0.1:8000/health` |
| .env present | `test -f /home/ubuntu/vernal-agents-post-v0/.env && echo OK` |
| Recent errors | `sudo journalctl -u vernal-agents --since "30 min ago" \| grep -E "ERROR\|CRITICAL\|Traceback"` |
| Nginx proxy timeouts | `sudo nginx -T \| grep timeout` |
| DB connectivity | From app dir: `python3 -c "from database import DatabaseManager; DatabaseManager(); print('OK')"` |
