...
export const loginUser = async ({
  ...
  try {
    ...
    console.log("[v0] Login payload being sent:", payload)
    ...
    if (response?.access_token || response?.token) {
      ...
      return {
        status: "error",
        message: response?.message || response?.error || "Unexpected login response",
      }
    }
  } catch (error: any) {
    console.error("[v0] Error during login - full error:", error)
    ...
    let errorMessage = "User does not exist. Please register first."

    if (error?.response?.data?.detail && Array.isArray(error?.response?.data?.detail)) {
      // Extract messages from the detail array
      const messages = error.response.data.detail
        .map((err: any) => err.msg || err.message || "Unknown error")
        .join(", ")
...
export const storeClaudeKey = async (apiKey: string): Promise<any> => {
  try {
    ...
    const response = await Service("store_claude_key", "POST", { api_key: apiKey }, undefined, true)
    ...
    if (response?.status === "success") {
      ...
      const errorMessage = response?.detail?.[0]?.msg || response?.message || "Failed to store Claude API key"
...
export const storeOpenAIKey = async (apiKey: string): Promise<any> => {
  try {
    ...
    const response = await Service("store_openai_key", "POST", { api_key: apiKey }, undefined, true)
    ...
    if (response?.status === "success") {
      ...
      const errorMessage = response?.detail?.[0]?.msg || response?.message || "Failed to store OpenAI API key"
